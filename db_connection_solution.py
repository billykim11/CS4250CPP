...
def count_term(term_list, check_term):
    count = 0
...
def createDocument(cur, docId, docText, docTitle, docDate, docCat):
    ...
    for term in found_terms:
        cur.execute("select id from public.term where term = %s", (str(term),))
        result = cur.fetchone()
        if not result:
            cur.execute("Insert into public.term (term, num_char) Values (%s, %s)", [str(term), len(term)])
    ...
    for rlt in results:
        term_count = count_term(found_terms, str(rlt['term']))
        if term_count > 0:
